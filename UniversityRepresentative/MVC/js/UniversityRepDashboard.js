...
function updatePassword() {
    ...
    if (!current || !newPass || !confirm) { showMessage(msgId, '⚠ Fill all fields.', 'error'); return; }
    if (newPass !== confirm) { showMessage(msgId, '⚠ Passwords mismatch.', 'error'); return; }
    if (newPass.length < 6) { showMessage(msgId, '⚠ Min 6 chars.', 'error'); return; }
    ...
    .then(r => r.json())
    .then(d => {
        if (d.status === 'success') { showMessage(msgId, '✔ ' + d.message, 'success'); resetPasswordForm(); }
        else { showMessage(msgId, '⚠ ' + d.message, 'error'); }
    })
    .catch(() => showMessage(msgId, '⚠ Server error.', 'error'));
}
...
function renderCourses(data) {
    ...
    if (data.length === 0) {
        tbody.innerHTML = '<tr><td colspan="3" style="text-align:center; padding:1rem;">No courses found.</td></tr>';
        return;
    }
